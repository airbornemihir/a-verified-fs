...
(defthm
  ...
  :hints
  (("subgoal *1/6.3"
    :in-theory (disable l4-wrchs-returns-stricter-fs-lemma-31)
    :use (:instance l4-wrchs-returns-stricter-fs-lemma-31
                    (name (car hns))))
   ("subgoal *1/5.3"
    :in-theory (disable l4-wrchs-returns-stricter-fs-lemma-31)
    :use (:instance l4-wrchs-returns-stricter-fs-lemma-31
...
(defthm l4-wrchs-returns-stricter-fs-lemma-34
  (implies
   (and (symbol-listp hns)
        (l3-fs-p fs)
        (boolean-listp alv)
   ...
   (equal
    ...
    (len
     (make-blocks
      (insert-text (unmake-blocks (fetch-blocks-by-indices disk (car entry))
                                  (cdr entry))
                   start text))))) :hints (("Goal'" :do-not-induct t :use (:instance make-blocks-correctness-4 (text1
        (insert-text (unmake-blocks (fetch-blocks-by-indices disk (car entry))
                                    (cdr entry))
                     start text)) (text2
        (insert-text nil
