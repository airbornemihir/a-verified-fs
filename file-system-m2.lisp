          ...
          (if (m1-regular-file-p (cdr head))
              (b*
                  ((contents (m1-file->contents (cdr head)))
                   ...
                   (indices
                    (stobj-find-n-free-clusters
                     fat32-in-memory (ceiling file-length
                                              (cluster-size fat32-in-memory))))
                   ;; I would have liked to do both these bindings together
                   ;; because that's what makes sense logically, but the
                   ;; syntactic restrictions on single-threaded objects make
                   ;; this awkward, albeit possible.
                   (fat32-in-memory
                    (if
                        (consp indices)
                        (stobj-set-indices-in-fa-table
                   ...
                   (dir-ent
                    (if
                      ...
                      (dir-ent-set-first-cluster-file-size
                       ...
                       0
                       file-length)))
                   (clusters (make-clusters contents (cluster-size fat32-in-memory)))
                   (fat32-in-memory
                    (stobj-set-clusters
                     clusters indices fat32-in-memory)))
                (mv fat32-in-memory dir-ent))
            (b*
                ((contents (m1-file->contents (cdr head)))
...
(defthm
  ...
  :hints (("Goal" :in-theory (disable len fat32-in-memoryp (:DEFINITION
