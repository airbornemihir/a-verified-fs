...
(defund
  ...
  (b*
      ((dir-ent (dir-ent-fix dir-ent))
       ((unless (mbt (< (nfix first-cluster) (fat-length fat32-in-memory))))
...
(encapsulate
  ...
  (verify-guards
    ...
    (("goal"
      ...
      :use
      (:instance
       (:rewrite find-n-free-clusters-correctness-4)(n 1)
       (fa-table
...
(defthmd
  ...
  :hints (("Goal" :do-not-induct t
           ...
           :expand (:free (FAT32-IN-MEMORY x y ENTRY-LIMIT)
                          (FAT32-IN-MEMORY-TO-M1-FS-HELPER FAT32-IN-MEMORY
...
(defthm
  m1-fs-to-fat32-in-memory-inversion-lemma-72
  (implies
   (and
    (compliant-fat32-in-memoryp fat32-in-memory)
    (integerp x)
  ...
  ((:in-theory (e/d nil
   ...
   :top :bash :bash (:demote 14)
   (:dive 1 1 1 1)
   := (:drop 9)
   :top
   (:in-theory (disable (:rewrite find-n-free-clusters-correctness-4)))
   (:use
    (:instance
     (:rewrite find-n-free-clusters-correctness-4)
     (n 1)
     ...
     (compliant-fat32-in-memoryp
      (stobj-set-indices-in-fa-table
       ...
       (find-n-free-clusters
        ...
        1)
       '(268435455)))
     (stringp (m1-file->contents (cdr (car fs))))
     (not
      (equal
       (fat32-entry-mask
        (fati
