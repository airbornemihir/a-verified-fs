...
(defthm
  ...
  (("goal"
   ...
   ("subgoal *1/1"
    :in-theory
    (e/d (m1-fs-to-fat32-in-memory-inversion-lemma-16)
         (fat32-in-memoryp get-clusterchain-contents
                           mod (:definition take-redefinition)
                           (:rewrite len-when-dir-ent-p)
                           (:rewrite unsigned-byte-listp-when-dir-ent-p)
...
(defthm m1-fs-to-fat32-in-memory-inversion-lemma-12
  (implies (not (zp x))
           (iff (< (binary-* x (len y)) x)
                (atom y))))

(defthmd m1-fs-to-fat32-in-memory-inversion-lemma-21
...
(defthm
  m1-fs-to-fat32-in-memory-inversion-lemma-40
...
(defthm
  m1-fs-to-fat32-in-memory-inversion-lemma-41
  ...
  (("goal"
    :in-theory
    (disable m1-fs-to-fat32-in-memory-inversion-lemma-36)
    :use (:instance m1-fs-to-fat32-in-memory-inversion-lemma-36
                    (x nil)))))

(defthm
  m1-fs-to-fat32-in-memory-inversion-lemma-42
  (implies (and (m1-file-alist-p contents1)
                (m1-file-no-dups-p contents1)
...
(defthm
  m1-fs-to-fat32-in-memory-inversion-lemma-43
  (implies
...
(defthm
  m1-fs-to-fat32-in-memory-inversion-lemma-44
  (implies (and (m1-file-alist-p contents1)
                (m1-file-no-dups-p contents1)
...
(defthm
  m1-fs-to-fat32-in-memory-inversion-lemma-45
  (implies
...
(defthm m1-fs-to-fat32-in-memory-inversion-lemma-47
...
;; This needs to be moved to fat32.lisp
(defthm
...
;; This needs to be moved to file-system-m1.lisp
...
(defthm
  ...
  :hints
  (("goal"
    :in-theory (disable (:definition fat32-in-memoryp)
                        dir-ent-fix
                        (:definition get-clusterchain-contents)
                        (:rewrite by-slice-you-mean-the-whole-cake-2)
                        (:definition fat32-build-index-list))
...
;; This should go to fat32.lisp
(defthm
  m1-fs-to-fat32-in-memory-inversion-lemma-59
  (implies
   (and
...
(defthm
  m1-fs-to-fat32-in-memory-inversion-lemma-60
  (implies
   (and (compliant-fat32-in-memoryp fat32-in-memory)
        (natp n)
        (equal (mv-nth 3
...
(defthm
  get-clusterchain-contents-of-place-contents
  (implies
   (and
    (compliant-fat32-in-memoryp fat32-in-memory)
    (natp length)
...
(defthmd
    ...
    (implies
     (and (compliant-fat32-in-memoryp fat32-in-memory)
          ...
          (<= (+ 2 (count-of-clusters fat32-in-memory))
              (fat-length fat32-in-memory)))
     (let
      ((mv (m1-fs-to-fat32-in-memory-helper
            fat32-in-memory (cdr fs)
            current-dir-first-cluster))
       (fs (cdr fs)))
      (let
  ...
  (("goal"
    ...
    :in-theory
    (e/d
     (place-contents lower-bounded-integer-listp)
     (nth
      ...
      (:rewrite m1-fs-to-fat32-in-memory-inversion-lemma-10)))
    :use
    (:instance
     (:rewrite m1-fs-to-fat32-in-memory-inversion-lemma-10)
     (index-list
      (append
       ...
       (find-n-free-clusters
        ...
        (+
         -1
         (len
          (make-clusters (m1-file->contents (cdr (car fs)))
                         (cluster-size fat32-in-memory)))))))
     (fat32-in-memory
      (stobj-set-indices-in-fa-table
       (mv-nth 0
       ...
       (find-n-free-clusters
        ...
        1)
       '(268435455)))
     (text (m1-file->contents (cdr (car fs))))))))

(defthm
  m1-fs-to-fat32-in-memory-inversion-lemma-66
  t
  :rule-classes
  ((:rewrite
    :corollary
...
(defthm
  m1-fs-to-fat32-in-memory-inversion-lemma-68
 (implies
  (and
   (equal (mv-nth 2 (place-contents
                     fat32-in-memory dir-ent
                     contents file-length first-cluster)) 0)
   (NOT (zp (CLUSTER-SIZE FAT32-IN-MEMORY)))
  ...
  (equal
   (place-contents
    fat32-in-memory dir-ent
    contents file-length first-cluster)
   (mv (stobj-set-indices-in-fa-table
        (stobj-set-clusters
         ...
         (list*
          ...
          (stobj-find-n-free-clusters
           fat32-in-memory
           (- (len (make-clusters contents (cluster-size fat32-in-memory)))
              1)))
         fat32-in-memory)
        (list*
         first-cluster
         (stobj-find-n-free-clusters
          fat32-in-memory
          (- (len (make-clusters contents (cluster-size fat32-in-memory))) 1)))
        (binary-append
         (stobj-find-n-free-clusters
          fat32-in-memory
          (- (len (make-clusters contents (cluster-size fat32-in-memory))) 1))
...
(defthm
  m1-fs-to-fat32-in-memory-inversion-lemma-69
  (implies
   (and
    (equal
     (mv-nth
 ...
 (defthmd lemma-1-4
   (implies
    (and
     ...
     (m1-file-no-dups-p (cdr fs))
     (consp (car fs))
     (not (assoc-equal (car (car fs)) (cdr fs)))
      ...
      (or
       ...
       (and
        ...
        (m1-dir-equiv
         ...
         fs)))))
   :hints (("goal"
            :in-theory (disable (:DEFINITION GET-CLUSTERCHAIN-CONTENTS)
