...
\begin{frame}{Why we need a verified filesystem}
  ...
  \item It would be worthwhile to verify that the
...
\begin{frame}{What we need}
  \begin{itemize}
    ...
    \item Yet, parsimony and avoidance of redundancy are essential for
      theorem proving.
    \item How about emulating the VFS interface?
    \item That would
...
\begin{frame}{Modelling a filesystem}
  ...
  \item The \texttt{strip-cars} would contain the file/directory
    names.
  \item Next, we'll look at a running example, and add/delete files
    from such a model.
  \end{itemize}
\end{frame}
