...
;; The following lemmas are redundant with the definitions in
;; books/std/io/base.lisp, from where they were taken with thanks.
(defthm
...
(defthm
  ...
  (implies (and (open-channels-p x) (assoc k x))
           (open-channel1 (cdr (assoc k x))))
  :hints
  (("goal"
    :in-theory (enable open-channels-p ordered-symbol-alistp
                       open-channel-listp))))
(defthm
  ...
  (("goal"
    :in-theory
    (e/d (state-p1)
         (open-channel1 open-channels-p
                        open-channels-p ordered-symbol-alistp
                        plist-worldp symbol-alistp
                        timer-alistp known-package-alistp
                        true-listp 32-bit-integer-listp
                        integer-listp readable-files-p
                        written-files-p read-files-p
                        writeable-files-p true-list-listp
                        all-boundp nth update-nth)))))
