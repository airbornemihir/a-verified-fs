...
(encapsulate
  ...
  ;; Hypotheses are minimised. Also, this theorem is the right vehicle for
  ;; applying these two lemmas with the expand hint - trying to instantiate
  ;; these lemmas the exact same way in the abs-find-file-correctness-lemma-40
  ;; and abs-find-file-correctness-lemma-37 simply does not work, and hours of
  ;; trying to find the place where these proofs diverge yield no result.
  (defthm
    ...
    (implies
     (and
      ...
      (no-duplicatesp-equal
       (append (remove-equal (1st-complete (frame->frame frame))
                             (strip-cars (frame->frame frame)))
      ...
      (prefixp
       ...
       (fat32-filename-list-fix path))
      (equal
       (true-list-fix (remove-assoc-equal (1st-complete (frame->frame frame))
                                          (frame->frame frame)))
       (remove-assoc-equal (1st-complete (frame->frame frame))
                           (frame->frame frame)))
      (no-duplicatesp-equal (strip-cars (frame->frame frame)))
   ...
   (implies
    ...
    :hints (("goal" :do-not-induct t :in-theory
             '((:compound-recognizer natp-compound-recognizer)
               ...
               (:congruence fat32-filename-list-equiv-implies-equal-len-1)
  ...
  (defthm abs-find-file-correctness-lemma-37
    (implies
     ...
     (equal
      ...
      (m1-regular-file-p (mv-nth 0 (abs-find-file frame path)))))
    :hints (("Goal" :do-not-induct t :in-theory
             (disable abs-find-file-correctness-lemma-10))))
