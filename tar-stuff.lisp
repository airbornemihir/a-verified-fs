...
(defthm
  ...
  (implies (and (not (integerp (- start)))
                (>= start 0))
           (> (len x) (- start)))
  :rule-classes :linear)

(defthm subseq-of-implode-of-append
  (equal (subseq (implode (append x y)) start end)
         (cond ((and (not (integerp (- (+ (len x) (len y)) start))) (null end)) "")
               ((and (not (integerp start)) (<= (len x)
                                                (- (+ (len x) (len y)) start))
                     (integerp (- (+ (len x) (len y)) start))
                     (null end))
                (implode (append (make-character-list x) (take (- (len y) start)
                                                               (make-character-list y)))))
               ((and (integerp start) (< (len x) start) (null end))
                (implode (take (- (+ (len x) (len y)) start) (nthcdr (+ start (- (len x)))
                                                                     (make-character-list y)))))
               ((and (integerp start) (< start 0) (null end))
                (implode (append (make-character-list x) (take (- (len y) start)
                                                               (make-character-list y)))))
               ((and (not (integerp start)) (integerp (- (+ (len x) (len y)) start))
                     (< (- (+ (len x) (len y)) start)
                        (len x))
                     (null end))
                (implode (take (- (+ (len x) (len y)) start) x)))
               ((null end) (implode (append (nthcdr start (make-character-list x))
                                            (make-character-list y))))
               ((stringp y) (implode (take (- end start) (nthcdr start (make-character-list x)))))
               ((not (natp (- end start))) "")
               ((and (< start 0) (< (- end start) (len x))) (implode (take (- end start) x)))
               ((and (not (integerp start)) (> (len x) (- end start)))
                (implode (take (- end start) x)))
               ((not (integerp start)) (implode (append (make-character-list x)
                                                        (take (- end (+ start (len x)))
                                                              (make-character-list y)))))
               ((and (<= 0 start) (<= end (len x)) (not (integerp end)))
                (implode (take (- end start) x)))
               ((and (< start 0) (<= (len x) (- end start)))
                (implode (append (make-character-list x)
                                 (take (- end (+ start (len x)))
                                       (make-character-list y)))))
               ((< start 0) (implode (take (- end start) x)))
               ((< (len x) start) (implode (take (- end start) (nthcdr (- start (len x))
                                                                       (make-character-list y)))))
               ((>= (len x) end) (implode (take (- end start)
                                                (nthcdr start (make-character-list x)))))
               (t (implode (append (nthcdr start (make-character-list x))
                                   (take (- end (len x)) (make-character-list y)))))))
  :hints (("goal" :in-theory (e/d (subseq subseq-list take) ((:e force)
                                                             take-of-too-many take-when-atom))
           :do-not-induct t
           :use ((:theorem (equal (+ start (- start) (- (len x))) (- (len x))))
                 (:theorem (equal (+ (len x) (- (len x)) (len y)) (len y)))))))
...
(thm (equal (subseq (implode (append x y))
                    start end)
            (cond ((and (not (integerp (+ (+ (len x) (len y)) (- start))))
                  ...
                  (t (implode (append (nthcdr start x)
                                      (take (+ end (- (len x))) y))))))
     :hints (("goal" :do-not-induct t
              :in-theory (disable subseq-of-implode-of-append)
              :use subseq-of-implode-of-append)))
