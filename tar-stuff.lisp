...
(thm (implies (and (< start 0)
     ...
     :hints (("goal" :in-theory (e/d (subseq subseq-list take)
                                     ((:e force) take-of-too-many))
              :do-not-induct t
              :use (:theorem (equal (+ (len x) (- (len x)) (len y))
                                    (len y))))))
...
(defthm
  ...
  (equal (subseq (implode (append x y))
                 start end)
         (cond ((and (stringp y) (< start 0) (null end))
                (implode (take (- (len x) start)
               ...
               ((and (not (consp x)) (null end))
                (implode (take (- (len y) start)
                               (nthcdr start (make-character-list y)))))
               ((and (< start 0)
                     (integerp start)
                     (null end))
               ...
               ((and (integerp (- end start))
                ...
                (implode (append (make-character-list x)
                                 (take (- end (+ start (len x)))
                                       (make-character-list y)))))
               ((stringp y)
                (subseq (implode x) start end))
               ((and end (not (stringp y))
                     (<= 0 start)
                     (<= end (len x))
                     (not (integerp end)))
                (implode (take (- end start) x)))
               ...
               ((and (< start 0)
                ...
                (implode (append (make-character-list x)
                                 (take (- end (+ start (len x)))
                                       (make-character-list y)))))
               ((and (< start 0) (not (null end)))
                (implode (take (- end start) x)))
               ((and (null end) (< (len x) start))
                (implode (nthcdr (- start (len x))
                                 (make-character-list y))))
               ((null end)
                (implode (append (nthcdr start (make-character-list x))
                                 (make-character-list y))))
               ((< (len x) start)
                (implode (take (- end start)
                               (nthcdr (- start (len x))
