  ...
  (equal
   ...
   (cond ((and (not (integerp (- (+ (len x) (len y)) start)))
         ...
         ((and (not (integerp start))
               ...
               (null end))
          (implode (append x (subseq-list y 0 (- (len y) start)))))
         ((and (integerp start)
               (< (len x) start)
               (null end))
          (implode (subseq-list y (- start (len x))
                                (len y))))
         ((and (integerp start)
               (< start 0)
               (null end))
         ...
         ((and (not (integerp start))
               ...
               (< (- (+ (len x) (len y)) start)
                  (len x))
               (null end))
          (implode (subseq-list x 0 (- (+ (len x) (len y)) start))))
         ((null end)
          (implode (append (subseq-list x start (len x))
                           y)))
         ((not (natp (- end start))) "")
         ((and (not (integerp start))
               (< (- end start) (len x)))
          (implode (subseq-list x 0 (- end start))))
         ((not (integerp start))
          (implode (append x
                           (subseq-list y 0 (- end (+ start (len x)))))))
         ((and (< start 0)
               (<= (len x) (- end start)))
          (implode (append x
                           (subseq-list y 0 (- end (+ start (len x)))))))
         ((< (len x) start)
          (implode (subseq-list y (- start (len x))
                                (- end (len x)))))
         ((<= end (len x))
          (implode (subseq-list x start end)))
         (t (implode (append (subseq-list x start (len x))
                             (subseq-list y 0 (- end (len x))))))))
