...
;; Rename later.
(defthm abs-pwrite-correctness-lemma-2
...
(defthm
  ...
  (("goal"
    :do-not-induct t
    :in-theory (e/d (frame-reps-fs good-frame-p abs-pwrite collapse
                                   1st-complete dist-names abs-separate
                                   abs-fs-fix len-of-insert-text))
    :expand
    (:with
     abs-pwrite-correctness-lemma-1
     (:free
...
(defthm
  ...
  (implies
   ...
   (equal
    ...
    0))
  :hints (("goal" :in-theory (enable hifat-place-file
                                     hifat-no-dups-p hifat-find-file
                                     abs-mkdir-correctness-lemma-30
                                     abs-mkdir-correctness-lemma-129)
           :do-not-induct t)))
...
(defthm abs-pwrite-correctness-lemma-31
  (implies
   (and
    (equal (frame-val->src (cdr (assoc-equal 0 frame)))
           0)
...
(defthm
  abs-pwrite-correctness-lemma-34
  (implies
   (and
    (equal
     (mv-nth
...
(defthm abs-pwrite-correctness-lemma-43
  (implies (and (consp x)
...
(defthm
  ...
  :hints
  (("goal"
    :in-theory (disable (:rewrite abs-pwrite-correctness-lemma-48))
    :use
    (:instance
...
(defthm
  abs-pwrite-correctness-lemma-58
  (implies
   (and (stringp buf)
        (integerp offset)
        (<= 0 offset)
  ...
  (("goal"
    :do-not-induct t
    :expand (:with m1-file-contents-fix-when-m1-file-contents-p
                   (:free (x) (m1-file-contents-fix x)))
    :in-theory
    (e/d (frame-reps-fs good-frame-p
                        abs-pwrite 1st-complete frame-addrs-root
                        dist-names abs-separate abs-fs-fix
                        assoc-equal-of-frame-with-root
                        hifat-no-dups-p
                        hifat-place-file hifat-find-file
...
(defthm
  ...
  (("goal"
    :do-not-induct t
    :in-theory (disable abs-mkdir-correctness-lemma-87
                        path-clear-when-prefixp)
    :use
    ((:instance
      path-clear-when-prefixp
      (x (frame-val->path (cdr (assoc-equal (abs-find-file-src frame path)
                                            frame))))
      (y path))
     abs-mkdir-correctness-lemma-87))))
...
(defthm abs-pwrite-correctness-lemma-61
  ...
  (implies
   (and
    ...
    (< (+ offset (len (explode buf)))
       4294967296)
    (consp
     (file-table-element->fid (cdr (assoc-equal (cdr (assoc-equal fd fd-table))
                                                file-table))))
    (consp
     (assoc-equal
      (basename (file-table-element->fid
...
(defthm
  abs-pwrite-correctness-lemma-67
  (implies
   (and
    (mv-nth 1 (collapse frame))
    (not (frame-val->path (cdr (assoc-equal 0 frame))))
...
(defthm
  abs-pwrite-correctness-lemma-77
  (implies
   (and
    (mv-nth 1 (collapse frame))
    (not (frame-val->path (cdr (assoc-equal 0 frame))))
...
(defthm
  abs-pwrite-correctness-lemma-79
  (implies
   (and
    (equal
     (len
...
(defthm
  abs-pwrite-correctness-lemma-81
  (implies
   (and
    (mv-nth 1 (collapse frame))
    (not (frame-val->path (cdr (assoc-equal 0 frame))))
  ...
  :hints
  (("goal"
    :in-theory (e/d (abs-pwrite-correctness-lemma-41)
                    ((:rewrite abs-mkdir-correctness-lemma-86)))
    :use
    (:instance
     (:rewrite abs-mkdir-correctness-lemma-86)
