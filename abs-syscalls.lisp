...
(defthmd
  ...
  :hints (("goal" :in-theory (enable frame->frame frame->root frame-p)
           :do-not-induct t
           :use (:instance when-atom-of-remove-assoc (x 0)
                           (alist frame))
      ...
      (and
       (<
        (collapse-1st-index
         frame
         (car (frame-addrs-before frame x (collapse-1st-index frame x))))
        (nfix (collapse-1st-index frame x)))
       (consp
        (assoc-equal
         (car (frame-addrs-before frame x (collapse-1st-index frame x)))
         (frame->frame frame)))
     ...
     (collapse-1st-index
      ...
      (car
       (chain-to-complete
        ...
        (car (frame-addrs-before frame x (collapse-1st-index frame x)))
        (cons x acc)))))))
  :instructions (:promote (:dive 1) := :up :x :bash))
...
(defthm
   ...
   (and
    (equal
     ...
     (car (chain-to-complete
           ...
           (car (frame-addrs-before frame x (collapse-1st-index frame x)))
           (cons x acc))))
    (consp (frame-addrs-before frame x (collapse-1st-index frame x)))
    (mv-nth 1 (collapse frame))
    (no-duplicatesp-equal (strip-cars (frame->frame frame)))
    (frame-p frame)
    (not (member-equal x acc))
  ...
  (("goal"
    :do-not-induct t
    :in-theory (disable (:rewrite chain-ends-in-abs-complete-lemma-3))
    :use
    (:instance
