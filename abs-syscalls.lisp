...
(defthm
  ...
  :hints
  (("goal"
    :in-theory (e/d (names-at) (abs-mkdir-correctness-lemma-47))
    :use
    (:instance
...
(defthm
  ...
  (implies
   (and
    (frame-p frame)
    (no-duplicatesp-equal (strip-cars frame))
    (atom (frame-val->path (cdr (assoc-equal (abs-find-file-src frame path)
                                             frame))))
  ...
  (("goal"
    ...
    (:instance
     abs-mkdir-correctness-lemma-86
     (n (len (frame-val->path (cdr (assoc-equal (abs-find-file-src frame path)
                                                frame)))))))))
      ...
      (member-equal
       ...
       (abs-addrs
        (frame-val->dir
         (cdr (assoc-equal 0
                           (partial-collapse frame (dirname path))))))))
     (prefixp (frame-val->path (cdr (assoc-equal 0 frame)))
              (dirname path))
     (not
      (equal
       (mv-nth
