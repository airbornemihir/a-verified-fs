...
;; For this function, it might be worth investing in the abs-fs-fix
...
(defthm
  ...
  (:promote
    ...
    (and
     (not
      (intersectp-equal
       (abs-addrs root)
       (abs-addrs
        (abs-fs-fix (frame-val->dir (cdr (assoc-equal (1st-complete frame)
                                                      frame)))))))
     (abs-fs-p root)
   ...
   (:bash ("goal" :in-theory (enable intersectp-equal)))))
...
(defthm
  ...
  (equal (abs-separate (frame-with-root root frame))
         (and (abs-no-dups-p (abs-fs-fix root))
