...
;;            :in-theory (enable abs-find-file abs-separate
;;                               collapse-src-path collapse-src-dir))
;;           ("subgoal *1/2.2" :expand (REMOVE-ASSOC-EQUAL X FRAME))))
...
(defthm
  ...
  :hints
  (("goal"
    :in-theory (disable abs-find-file-correctness-1-lemma-46)
    :use
    (:instance
     abs-find-file-correctness-1-lemma-46
     (name
   ...
   (equal
    (mv-nth
     ...
     (abs-find-file-helper
      ...
      (nthcdr (len (frame-val->path$inline (cdr (assoc-equal x frame))))
              pathname)))
    *enoent*))
  :hints (("goal" :in-theory (enable abs-find-file))))
...
(defthm
  ...
  (implies
   (and (equal (mv-nth 1 (collapse root frame))
               t)
        (distinguish-names root nil frame)
        (consp (assoc-equal x frame))
        (equal (mv-nth 1 (abs-find-file-helper root pathname))
...
(thm
       ...
       (frame-val
        ...
        (context-apply
         ...
         (frame-val->dir (cdr (assoc-equal (abs-find-first-complete frame)
                                           frame)))
         (abs-find-first-complete frame)
         (nthcdr (len (collapse-src-path frame))
                 (frame-val->path
                  (cdr (assoc-equal (abs-find-first-complete frame)
                                    frame)))))
        (frame-val->src
 ...
 :hints
 (("goal"
   :in-theory (e/d (collapse-src-path collapse-src-dir)
...
(verify
 ...
 (:promote
  ...
  (:claim
   (and
    (m1-regular-file-p
     (mv-nth 0
             (abs-find-file-helper root
                                   (abs-enotdir-witness root pathname))))
    (equal
     (mv-nth 1
             (abs-find-file-helper root
                                   (abs-enotdir-witness root pathname)))
     0)))
  (:claim (prefixp (abs-enotdir-witness root pathname)
                   pathname))
  (:dive 1)
  (:rewrite car-of-nthcdr)
  (:casesplit
   (and
    (not (list-equiv (abs-enotdir-witness root pathname)
                     pathname))
    (<
     (nfix
      (len
       (frame-val->path (cdr (assoc-equal (abs-find-first-complete frame)
                                          frame)))))
     (len (abs-enotdir-witness root pathname)))
    (prefixp
     (frame-val->path (cdr (assoc-equal (abs-find-first-complete frame)
                                        frame)))
     (abs-enotdir-witness root pathname))))
  (:rewrite nth-when-prefixp
            ((x (abs-enotdir-witness root pathname))))
  :top (:dive 2 2)
  (:=
   (take
    (len
  ...
  (:claim
   (and
    (fat32-filename-list-p (abs-enotdir-witness root pathname))
    (<
     (len
      (frame-val->path (cdr (assoc-equal (abs-find-first-complete frame)
                                         frame))))
  ...
  (:bash
   ("goal"
    :in-theory (e/d (list-equiv abs-find-file-helper)
                    ((:rewrite prefixp-one-way-or-another)))
    :use
    (:instance
     (:rewrite prefixp-one-way-or-another)
     (x (frame-val->path (cdr (assoc-equal (abs-find-first-complete frame)
                                           frame))))
     (y (abs-enotdir-witness root pathname))
     (z pathname))))
  (:claim
   (abs-directory-file-p
    (mv-nth
  ...
  (:bash ("goal" :expand (collapse root frame)))
  (:change-goal nil t)
  (:change-goal nil t)
  (:claim
   (m1-regular-file-p
  ...
  (:claim
   (and
    (consp (abs-enotdir-witness root pathname))
    (zp
     (mv-nth
      1
      (abs-find-file-helper
   ...
   (and
    ...
    (not
     (fat32-filename-list-equiv
      ...
      (frame-val->path (cdr (assoc-equal (abs-find-first-complete frame)
                                         frame)))))))
  (:rewrite abs-find-file-correctness-1-lemma-60
            ((pathname (abs-enotdir-witness root pathname))))
  :top :bash (:change-goal nil t)
  :top :bash
  (:change-goal (((((((main . 2) . 3) . 3) . 2) . 3) . 1)
                 . 2)
                t)
  (:contrapose 22)
  (:dive 2)
     ...
     (m1-regular-file-p
      (mv-nth
       ...
       (abs-find-file-helper
        ...
        (frame-val->path (cdr (assoc-equal (abs-find-first-complete frame)
                                           frame)))))))))
  (:rewrite abs-find-file-correctness-1-lemma-61)
  :top :s :bash
  (:=
   (and
    (not
     (consp
      (frame-val->path (cdr (assoc-equal (abs-find-first-complete frame)
                                         frame)))))
    (not (consp (abs-enotdir-witness root pathname)))))
  (:= nil)
  (:contrapose 21)
  (:dive 1)
  (:casesplit
...
(thm
 (implies (and  (abs-file-alist-p root)
          ...
          (equal (mv-nth 1 (abs-find-file frame pathname))
                 2))
 :hints (("goal" :in-theory (enable collapse abs-find-file)
          :induct (collapse root frame))))

;; OMG prove this with the correct substitutions.
(thm
 (implies
  (and
   (equal
   ...
   (distinguish-names
    ...
    (remove-assoc-equal (abs-find-first-complete frame)
                        frame))
   (abs-separate (remove-assoc-equal (abs-find-first-complete frame)
                                     frame))
   (equal
    (mv-nth
     1
