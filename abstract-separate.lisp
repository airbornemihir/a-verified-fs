...
(defcong absfat-equiv set-equiv (names-at fs relpath) 1
  ...
  (("goal"
    :in-theory (e/d (absfat-equiv set-equiv)
                    (absfat-equiv-implies-set-equiv-names-at-1-lemma-1))
    :use
    ((:instance absfat-equiv-implies-set-equiv-names-at-1-lemma-1
...
;; It would be nice for this to replace the original lemma at the top...
(defthm
    ...
    (:promote
     (:claim
      (and
       ...
       (dist-names
        (frame-val->dir (cdr (assoc-equal x (frame->frame frame))))
        (append (frame-val->path (cdr (assoc-equal x (frame->frame frame))))
                nil)
        (remove-assoc-equal
         x
         (remove-assoc-equal
     ...
     (:dive 2)
     :=
     :top (:rewrite abs-separate-correctness-1-lemma-22)))
       ...
       (("goal"
         ...
         (e/d (collapse abs-separate 1st-complete-src)
              ((:definition assoc-equal)
               ...
               (:definition len)
               (:rewrite remove-assoc-when-absent)
               (:rewrite abs-find-file-correctness-1-lemma-20) lemma-3)) :use
         lemma-3 :cases
         ((nthcdr
           (len (frame-val->path$inline (cdr (assoc-equal x (frame->frame frame)))))
           (frame-val->path$inline
            (cdr (assoc-equal (1st-complete (frame->frame frame))
                              (frame->frame frame)))))))))
